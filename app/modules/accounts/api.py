...
async def create_account(payload: AccountCreate, session: SessionDep) -> AccountOut:
...
async def update_account(account_id: str, payload: AccountUpdate, session: SessionDep) -> AccountOut:
...
async def delete_account(account_id: str, session: SessionDep) -> None:
    account = await session.get(Account, account_id)
    if account is None:
        raise HTTPException(404, "account not found")
